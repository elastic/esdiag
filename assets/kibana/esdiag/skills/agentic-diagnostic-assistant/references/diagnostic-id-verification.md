# Diagnostic ID Verification

## Purpose

Verifies that a diagnostic ID exists in Project ADA diagnostic data before diagnostic analysis proceeds.

## Required Input

- `diagnostic_id`: the diagnostic ID to verify, for example `cluster-name@YYYY-MM-DD~wxyz`.
...
| WHERE diagnostic.id == "{{diagnostic_id}}" AND @timestamp >= NOW() - 1 year
| KEEP diagnostic.id
| LIMIT 1
```
...
```text
/s/esdiag/app/dashboards#/view/diagnostic-id-verification?_g=(filters:!(('$state':(store:globalState),meta:(disabled:!f,index:a738e381-1538-4fb5-a7c1-eebbdff4623b,key:diagnostic.id,negate:!f,params:(query:'{{diagnostic_id}}'),type:phrase),query:(match_phrase:(diagnostic.id:'{{diagnostic_id}}')))),time:(from:now-1y%2Fd,to:now))
```
