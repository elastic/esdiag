...
The system SHALL expose a list of saved job names to the `/jobs` page UI. The list SHALL reflect the current contents of `jobs.yml` and update after every save or delete operation.

#### Scenario: Jobs listed on page load
- **WHEN** the user navigates to the `/jobs` page in User mode
...
The system SHALL restore a saved job's full workflow configuration into the `/jobs` page signal state when the user selects it from the left panel.
...
The system SHALL allow the user to delete a saved job by name from the `/jobs` page. Deletion SHALL remove the entry from `jobs.yml` and refresh the left-panel list.
...
The system SHALL provide `esdiag job run <name>` as a CLI subcommand that loads the named job from `~/.esdiag/jobs.yml` and executes it using the existing CLI collect/process pipeline.
...
The system SHALL provide `esdiag job delete <name>` as a CLI subcommand that removes the named job from `~/.esdiag/jobs.yml`.
