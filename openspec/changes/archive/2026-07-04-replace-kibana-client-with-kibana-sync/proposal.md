...
- Expand ESDiag's bundled Kibana assets into the `kibana-sync` filesystem bundle layout for spaces, saved objects, agents, tools, and workflows.
- Package the Kibana filesystem bundle into a single generated embedded bundle so release binaries do not embed each raw Kibana object file separately.
...
- **Bundled Kibana assets:** Existing dashboards and supporting saved objects are stored in the `kibana-sync` bundle layout and embedded as one generated bundle artifact.
