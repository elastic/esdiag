...
- [x] 1.1 Add `--zip` option parsing for `collect` as optional path/destination semantics (`Option<Path>`, default `.` when flag used without explicit path).
- [x] 1.2 Add `--zip` option parsing for `process` and define output target as `{diagnostic}.zip`.
...
- [x] 4.2 Add/update tests verifying `process --zip` writes `{diagnostic}.zip` with expected entries.
