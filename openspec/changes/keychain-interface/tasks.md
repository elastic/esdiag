...
- [x] 1.1 Update footer Datastar template/state so output selection only supports existing host selection and save
- [x] 1.2 Add `Edit Hosts` button next to save in footer controls and wire it to open the manager modal
...
- [ ] 3.1 Implement Datastar host manager modal for `KnownHost` create/update/delete with full editable fields
- [ ] 3.2 Implement backend handlers for host CRUD persistence in `hosts.yml` with validation and error responses
- [x] 3.3 Implement keychain metadata listing for modal auth selection using entry names only (no secret values in response)
- [ ] 3.4 Implement host auth binding to selected keychain entry name and validate referenced entry existence on save
...
- [ ] 4.1 Enforce backend-only secret handling so decrypted/ciphertext secret values are never serialized to frontend state
- [ ] 4.2 Add guardrails to require unlock before keychain-backed reads/writes and return actionable locked-state errors
...
- [ ] 4.5 On correct preflight password, unlock keystore and continue processing without requiring user to re-submit start action
- [ ] 4.6 On incorrect preflight password, keep processing blocked and invalidate password field for retry
- [ ] 4.7 Reject secure-host processing start with a keystore-unavailable error when feature is disabled or runtime mode is `service`
- [ ] 4.8 Ensure secure-host processing lifecycle refreshes session lease on each keystore-backed host request
...
- [ ] 5.1 Add tests for idempotent `/keystore/unlock` and `/keystore/lock` behavior and PatchSignals updates for `keystore.*`
...
- [ ] 5.10 Add tests proving frontend payloads contain keychain names/metadata only and never secret values
- [ ] 5.11 Add integration/UI tests for footer changes (`Edit Hosts`, output selector restrictions, no inline host creation)
