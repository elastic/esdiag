...
Defines how ESDiag's owned per-product `sources.yml` collection definitions are kept
current against upstream `support-diagnostics` catalogs: a field-level overlay that
absorbs upstream path and version changes while preserving ESDiag's own enrichments,
performed on a required recurring cadence.
