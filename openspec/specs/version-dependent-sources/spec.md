...
Defines the per-product `sources.yml` collection definition: how a source's endpoint and
output path resolve against the target version, and the execution metadata each entry
carries for collect and process scheduling.
