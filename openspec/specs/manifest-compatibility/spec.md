...
Defines how ESDiag stays compatible with bundle manifests it receives but does not own:
permanent backward read tolerance for `support-diagnostics` and prior ESDiag manifests,
additive-only evolution, and read-time inference in place of migration.
